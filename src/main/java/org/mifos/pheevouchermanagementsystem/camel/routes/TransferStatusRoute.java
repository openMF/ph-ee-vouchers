...
public class TransferStatusRoute extends BaseRouteBuilder {
    ...
    public void configure() throws Exception {
        from("direct:direct:send-transfer-query")
                .id("transfer-status")
                .process(exchange -> {
                ...
                .setHeader(
                        Exchange.HTTP_QUERY,
                        simple(new StringBuilder().append("size=1")
                                .append("page=0")
                                .append("clientCorrelationId=${exchangeProperty.clientCorrelationId}").toString()
                        )
                )
                .setProperty(HOST, simple("{{operations.hostname}}"))
                .setProperty(ENDPOINT, simple("/api/v1/transfers/"))
