...
import static org.mifos.pheevouchermanagementsystem.util.VoucherStatusEnum.EXPIRED;
import static org.mifos.pheevouchermanagementsystem.util.VoucherStatusEnum.SUSPENDED;
...
public class VoucherLifecycleService {
    ...
    @Autowired
    public VoucherLifecycleService(VoucherRepository voucherRepository, ErrorTrackingRepository errorTrackingRepository,
    ...
    public void suspendVoucher(RequestDTO request, String callbackURL, String registeringInstitutionId) {
        ...
        try {
            sendCallbackService.sendCallback(
                    objectMapper.writeValueAsString(new VoucherLifecycleCallbackResponseDTO(UniqueIDGenerator.generateUniqueNumber(12),
                            request.getRequestID(), failedCaseList.size(), failedCaseList)),
                    callbackURL);
    ...
    public void validateAndReactivateVouchers(List<VoucherInstruction> voucherInstructionList, RequestDTO request,
            List<FailedCaseDTO> failedCaseList, String registeringInstitutionId) {
        voucherInstructionList.stream().forEach(voucherInstruction -> {
            ...
            if (isValidated) {
                ...
                if (validateRegisteringInstitutionId(voucher, registeringInstitutionId)) {
                    ...
                    FailedCaseDTO failedCase = new FailedCaseDTO(voucherInstruction.getSerialNumber(),
                            "The voucher provided in the request does not correspond to the same registering ID.");
    ...
    public void validateAndSuspendVouchers(List<VoucherInstruction> voucherInstructionList, RequestDTO request,
            List<FailedCaseDTO> failedCaseList, String registeringInstitutionId) {
        voucherInstructionList.stream().forEach(voucherInstruction -> {
            String requestID = request.getRequestID();
    ...
    public void validateAndCancelVouchers(List<VoucherInstruction> voucherInstructionList, RequestDTO request,
            List<FailedCaseDTO> failedCaseList, String registeringInstitutionId) {
        voucherInstructionList.stream().forEach(voucherInstruction -> {
            String requestID = request.getRequestID();
    ...
    public Boolean validateVoucherForReactivate(VoucherInstruction voucherInstruction, RequestDTO request) {
        if (voucherRepository.existsBySerialNo(voucherInstruction.getSerialNumber())) {
            Voucher voucher = fetchVoucher(voucherInstruction);
            return !voucher.getStatus().equals(EXPIRED.getValue()) && !voucher.getStatus().equals(UTILIZED.getValue())
                    && !voucher.getStatus().equals(CANCELLED.getValue());
        }
        return false;
    }
    ...
    public Boolean validateVoucherForSuspend(VoucherInstruction voucherInstruction, RequestDTO request) {
        if (voucherRepository.existsBySerialNo(voucherInstruction.getSerialNumber())) {
            Voucher voucher = fetchVoucher(voucherInstruction);
            return !voucher.getStatus().equals(EXPIRED.getValue()) && !voucher.getStatus().equals(CANCELLED.getValue())
                    && !voucher.getStatus().equals(UTILIZED.getValue());
        }
        return false;
    }
    ...
    public Voucher fetchVoucher(VoucherInstruction voucherInstruction) {
        return voucherRepository.findBySerialNo(voucherInstruction.getSerialNumber())
                .orElseThrow(() -> VoucherNotFoundException.voucherNotFound(voucherInstruction.getSerialNumber()));
    }
}
