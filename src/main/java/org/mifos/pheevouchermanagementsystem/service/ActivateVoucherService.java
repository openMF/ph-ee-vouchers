...
public class ActivateVoucherService {
    ...
    @Autowired
    public ActivateVoucherService(VoucherRepository voucherRepository, ErrorTrackingRepository errorTrackingRepository,
    ...
    public void validateVouchers(List<VoucherInstruction> voucherInstructionList, RequestDTO request, List<FailedCaseDTO> failedCaseList,
        ...
        voucherInstructionList.stream().forEach(voucherInstruction -> {
            ...
            if (voucherValidation) {
                Voucher voucher = voucherRepository.findBySerialNo(voucherInstruction.getSerialNumber())
                ...
                FailedCaseDTO failedCase = new FailedCaseDTO(voucherInstruction.getSerialNumber(),
                        "Voucher with serial number does not exist.");
                failedCaseList.add(failedCase);
            }
        });
    }
    ...
    public void activateVouchers(VoucherInstruction voucherInstruction, List<FailedCaseDTO> failedCaseList, RequestDTO request) {
        try {
            ...
            if (validateVoucherStatus(voucher, failedCaseList)) {
                ...
                LocalDate updatedDate = LocalDate.now(ZoneId.systemDefault()).plusDays(expiryTime);
                voucher.setActivatedDate(LocalDateTime.now(ZoneId.systemDefault()));
