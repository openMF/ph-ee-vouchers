        ...
        zeebeClient.newWorker().jobType(CREATE_VOUCHERS.getValue()).handler((client, job) -> {
            ...
            if (status.equals("Y")) {
                ...
                for (VoucherInstruction voucherInstruction : voucherInstructions) {
                    createVoucherService.addVouchers(voucherInstruction, successfulVouchers, errorTrackingsList, requestId, batchId,
                            existingVariables.get("registeringInstitutionId").toString());
                }
                try {
